...
<?php
  ...
  $SQL = "SELECT * FROM preventa inner join clientes on preventa.CodCliente=clientes.Codigo WHERE (preventa.Empresa LIKE '$Empresa') AND (clientes.Empresa LIKE '$Empresa')";
    ...
    if(isset($_POST["submit"]))
    {
      ...
      $SQL = "SELECT * FROM preventa inner join clientes on preventa.CodCliente=clientes.Codigo WHERE (preventa.CodCliente LIKE '%$CodCliente%') AND (preventa.Empresa LIKE '$Empresa') AND (clientes.Empresa LIKE '$Empresa')";
...
<html lang="en">
<head>
  <title>Consultar Preventa ID</title>
  <meta charset="utf-8">
...
<nav class="navbar navbar-default" >
  ...
  <h2 style="color:grey">Preventas</h2>
      ...
      <form class="form-horizontal" role = "form" method = "post" action="consultarPreventaCodigo.php">

        <div class="row">
            <div class="col-xs-2">
              <label for = "Codigo" class = "control-label" style="color:white">Documento del cliente:</label> 
            </div>
            <div class="col-xs-3">
              <input class="form-control" type="text" id="CodCliente" name="CodCliente" placeholder="Documento del cliente"> 
            </div>            
        </div>
        <br>
        <div class = "row">
