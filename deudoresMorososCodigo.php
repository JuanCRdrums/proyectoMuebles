<?php
  ...
  {
    $Codigo = $_POST["Codigo"];
    $SQL1="SELECT clientes.Codigo, clientes.Nombre, clientes.Direccion, clientes.Telefono, cobro.Numero FROM cobro INNER JOIN clientes ON cobro.CodCliente = clientes.Codigo WHERE (clientes.Codigo LIKE '%$Codigo%') AND (clientes.Empresa LIKE '$Empresa') AND (cobro.Empresa LIKE '$Empresa') ORDER BY cobro.Numero";
...
<nav class="navbar navbar-default" >
  ...
  <form class="form-horizontal" role = "form" method = "post" action="deudoresMorososCodigo.php">

      <div class="row">
          <div class="col-xs-3"> 
            <label for = "Codigo" class = "control-label" style="color:white">Documento Cliente:</label>   
          </div>
          <div class="col-xs-3">
             <input class="form-control" type="text" id="Codigo" name="Codigo" placeholder="Documento Cliente" >
          </div>         
      </div>    
