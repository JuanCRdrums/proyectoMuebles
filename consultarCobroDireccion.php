  ...
  if(isset($_POST["submit"]))
    {
      ...
      $SQL = "SELECT cobro.Numero, cobro.FechaEntrega, cobro.FormaPago, cobro.Sena, cobro.Vendedor, articulos.Nombre, articulos.Color, articulos.Valor, cobro.CodCliente, clientes.Codigo  FROM ((cobro INNER JOIN articulos ON cobro.CodMueble = articulos.Codigo) INNER JOIN clientes ON cobro.CodCliente = clientes.Codigo) WHERE (clientes.Direccion LIKE '%$Direccion%') AND (cobro.Empresa LIKE '$Empresa') AND (articulos.Empresa LIKE '$Empresa') AND (clientes.Empresa LIKE '$Empresa') ORDER BY cobro.Numero ";

      $SQL1 = "SELECT clientes.Codigo, clientes.Nombre, clientes.Direccion, clientes.Telefono, cobro.Numero FROM cobro INNER JOIN clientes ON cobro.CodCliente = clientes.Codigo WHERE (clientes.Direccion LIKE '%$Direccion%') AND (clientes.Empresa LIKE '$Empresa') AND (cobro.Empresa LIKE '$Empresa') ORDER BY cobro.Numero ";
...
<html lang="en">
<head>
  <title>Consultar Cobro Dirección</title>
  <meta charset="utf-8">
...
<nav class="navbar navbar-default" >
  ...
  <form class="form-horizontal" role = "form" method = "post" action="consultarCobroDireccion.php">
        ...
        <div class="row">
            ...
            <div class="col-xs-3">
              <input class="form-control" type="text" id="Numero" name="Direccion" placeholder="Dirección cliente"> 
            </div>            
        </div>
        <br>
        <div class = "row">
