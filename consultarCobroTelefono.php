  ...
  if(isset($_POST["submit"]))
    {
      ...
      $SQL = "SELECT cobro.Numero, cobro.FechaEntrega, cobro.FormaPago, cobro.Sena, cobro.Vendedor, articulos.Nombre, articulos.Color, articulos.Valor,articulos.Codigo, cobro.CodCliente, clientes.Codigo  FROM ((cobro INNER JOIN articulos ON cobro.CodMueble = articulos.Codigo) INNER JOIN clientes ON cobro.CodCliente = clientes.Codigo) WHERE (clientes.Telefono LIKE '%$Telefono%') AND (cobro.Empresa LIKE '$Empresa') AND (clientes.Empresa LIKE '$Empresa') AND (articulos.Empresa LIKE '$Empresa') ORDER BY cobro.Numero ";

      $SQL1 = "SELECT clientes.Codigo, clientes.Nombre, clientes.Direccion, clientes.Telefono, cobro.Numero FROM cobro INNER JOIN clientes ON cobro.CodCliente = clientes.Codigo WHERE (clientes.Telefono LIKE '%$Telefono%') AND (clientes.Empresa LIKE '$Empresa') ORDER BY cobro.Numero ";
